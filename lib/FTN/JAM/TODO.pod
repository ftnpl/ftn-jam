...
as serving as an example of how to use FTN::JAM. And/or use them as the 
basis for additional tests in the t/ directory? Note that some of the basic
functions are already in the t/01-basic.t test file.

Having the global variable $Errnum and the package FTN::JAM::Errnum may
not a good idea, as referencing $FTN::JAM::Errnum or the error number
constant $FTN::JAM::Errnum::IO_ERROR might be ambigous. Change the 
name for the global variable and/or the package? Change the name of $Errnum
to $error_number? Change both the global varible as well as the module
package name (which is used for the constants definitions)?
...
One argument selects are used at multiple places in JAM.pm. It is being 
...
Move the TEST:* modules from the PREREQ_PM section in Makefile.PL to a new
BUILD_REQUIRES section.
...
but the return does not indicate that. It does set $Errnum to indicate an
...
'return 0;' to explicitly set that.  Then change the testing code as
neccessary to make use of that.
...
Add a description of the function, including the parameter it expects
...
The IO_ERROR return is using the 'return;' style, which is actually returning
the results of setting $Errnum which is a boolean true. Explicitly use
'return 0;' to return a boolean false so that the calling program knows to
check the $Errnum variable for why it failed.
...
Add a description of the function, including the parameter(s) it expects
and what it returns.
...
it returns:  it is being passed a reference to a hash that contains four keys,
...
The four code blocks that attempt to do the unlink are not explicitly setting
...
Explicitly set a local variable for the parameter that should be passed to the
...
Expand and update the POD documentation for the read only Subfields constants
in the file.


=head2 FTN::JAM::Attr Module

Expand and update the POD documentation for the read only Attributes constants
in the file.
...
so that it is easier to see where problems are happening?

And/or use them as the basis for additional tests in the t/ directory? Note
that some of the basic functions are already in the t/01-basic.t test file.

=head1 SEE ALSO

 L<FTN::JAM>, L<FTN::JAM::Subfields>, L<FTN::JAM::Attr>, L<FTN::JAM::Errnum>,
 and L<FTN::JAM::Tutorial>.
