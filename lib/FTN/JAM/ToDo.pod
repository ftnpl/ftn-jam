...
Verify/ensure that variables passed to subroutines are unpacked to local
variables before being used in the subroutine.

Localize usage of Magic variable "$|".

Verify/ensure that booleans being used in the same code are of the same
precedence.

Move FTN::JAM::Subfields, FTN::JAM::Attr, and FTN::JAM::Errnum into their
own module files?
...
Add test subroutines for createing and removing a message base:  for that,
need to figure out where to put temporary files used with/for/in testing.
Adapt some and/or all of the demo scripts as part of the testing?
